...
- Contact form (`contacto.html`) submits via `fetch` POST to `send.php`, which sends mail over Gmail SMTP (PHPMailer, bundled in `site/php/PHPMailer/`) to `contactoalmatriz@gmail.com`. SMTP credentials live **outside the webroot** at `/home/esc447/private/mail-config.php` (one level above `public_html`); `send.php` loads it via `dirname(__DIR__).'/private/mail-config.php'` (override with the `MAIL_CONFIG_PATH` env var). Create it once on the server by copying the committed template `site/php/mail-config.example.php`, setting a Google **App Password**, and `chmod 600`. It is never in git and never deployed. `site/php/.htaccess` still denies web access to the bundled PHPMailer library.
