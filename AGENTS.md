...
- Contact form (`contacto.html`) submits via `fetch` POST to `send.php`, which sends mail over Gmail SMTP (PHPMailer, bundled in `site/php/PHPMailer/`) to `contactoalmatriz@gmail.com`. SMTP credentials live in `site/php/mail-config.php`, which is **git-ignored**; only `mail-config.example.php` is committed. On the server this file must be created once (copy the example, set a Google **App Password**) — the deploy never overwrites it because it is untracked and the deploy no longer deletes files. `site/php/.htaccess` denies direct web access to the config/library.
